...
- Good homes here: the GPT-4o **extraction result** shape (person/topics/life-events/follow-ups), notification payload types, backup/export JSON schema, tone/category enums, the topic-expiry state union.
- Keep types that belong to a single feature inside that feature folder; only lift to `@/types` when shared.
- Types only — no runtime values. `kebab-case` files (e.g. `extraction.ts`, `backup.ts`).
